...
DEFAULT_PROJECT = REPO_ROOT / "models" / "yolo26x" / "runs"
DEFAULT_RUN_NAME = "cadica_selected_seed42_4090"
...
def default_batch_for_device(device: str) -> int:
    return 16 if device not in {"cpu", "mps"} else 4
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train YOLO26x on the prepared CADICA dataset.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Training batch size. Defaults to 16 on CUDA and 4 on Apple Silicon / CPU.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Data loader workers. Defaults to 12 on CUDA and 2 on Apple Silicon / CPU.",
    )
...
def main() -> None:
    ...
    cache = default_cache_for_device(device)
    ...
    try:
        ...
        raise SystemExit(
            ...
            "Create the uv env from models/yolo26x/README.md and reinstall dependencies."
        ) from error
