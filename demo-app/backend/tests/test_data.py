...
def test_bmp_frames_supported_and_empty_labels_mark_patient_as_labeled(tmp_path: Path) -> None:
    data_dir = tmp_path / "patients"
    ...
    (label_dir / "f1.txt").write_text("", encoding="utf-8")

    manifest = {
        "patients": [
            {"id": "p1", "framesDir": "p1/frames", "labelsDir": "p1/labels", "defaultFps": 12, "datasetId": "cadica"}
        ]
    }
    (data_dir / "manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    ...
    assert summary.datasetId == "cadica"
...
def test_ensure_labels_dir_and_mark_label_saved_for_unlabeled_patient(tmp_path: Path) -> None:
    ...
    manifest = {"patients": [{"id": "p1", "framesDir": "p1/frames", "defaultFps": 12, "datasetId": "cadica"}]}
    (data_dir / "manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
...
def test_manifest_defaults_to_cadica_bbox(tmp_path: Path) -> None:
    data_dir = tmp_path / "patients"
    _make_frame(data_dir / "p1" / "frames", "f1.png")
    manifest = {
        "patients": [
...
def test_manifest_rejects_non_cadica_dataset(tmp_path: Path) -> None:
    data_dir = tmp_path / "patients"
    _make_frame(data_dir / "p1" / "frames", "f1.png")
    manifest = {"patients": [{"id": "p1", "framesDir": "p1/frames", "datasetId": "mendeley"}]}
    (data_dir / "manifest.json").write_text(json.dumps(manifest), encoding="utf-8")

    with pytest.raises(ManifestValidationError, match="Unsupported datasetId"):
        PatientStore(data_dir)
